...
class EducationController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $data['google_links'] = array_values(array_filter(
            array_map(fn($l) => [
                'name' => trim($l['name'] ?? ''),
                'url'  => trim($l['url']  ?? ''),
            ], is_array($rawLinks) ? $rawLinks : []),
            fn($l) => !empty($l['url'])
        ));

        // 대표 이미지 업로드
        if ($request->hasFile('image')) {
            $uploadDir = dirname(base_path()) . '/uploads/educations';
        ...
        if ($request->hasFile('pdfs')) {
            $uploadDir = dirname(base_path()) . '/uploads/education_pdfs';
            if (!is_dir($uploadDir)) mkdir($uploadDir, 0755, true);
    ...
    public function update(Request $request, string $id)
    {
        ...
        if ($request->hasFile('image')) {
            if ($education->image_path) {
                $old = dirname(base_path()) . '/' . $education->image_path;
        ...
        foreach ($toRemove as $removePath) {
            $fullPath = dirname(base_path()) . '/' . $removePath;
    ...
    {
        $education = Education::findOrFail($id);
        $education->is_visible = !$education->is_visible;
        $education->save();
    ...
    {
        $query = Education::where('is_visible', true)
