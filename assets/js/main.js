document.addEventListener('DOMContentLoaded', function(){
    ...
    if(mpjSlideArea){
        ...
        // 외부(API 로드 후)에서 재초기화할 수 있도록 전역 노출
        window.initMpj = function(){
            mpjIndex = 0;
            mpjSyncInitial();
            mpjUpdateFraction();
            mpjStopAutoplay();
    ...
    if(mslTit && mslEffectBox && mslFolders.length && typeof gsap !== 'undefined' && typeof ScrollTrigger !== 'undefined'){
        ...
        var mslLang = (document.documentElement.lang || 'ko').toLowerCase().slice(0, 2);
        var mslEffectTxts = {
            ko: {
                step1: '목표 성능을 만족하기 위해 설계 수정과 해석이 반복됩니다. <br> 이 과정은 수십에서 수백 번의 Iteration을 발생시킵니다.',
                step2: 'AslanX가 반복적인 설계 탐색 과정을 AI로 내부에서 수행합니다.',
                step3: 'AI가 생성한 설계는 검증 단계로 전달됩니다. <br> 설계자는 반복 과정 없이 최적화된 결과를 바로 확인할 수 있습니다.',
                step4: 'AslanX는 AI 기반 설계 탐색을 통해 다양한 설계 후보를 생성하고 성능을 예측합니다. <br> AI는 생성·예측·최적화 과정을 반복하며 목표 조건을 만족하는 설계를 자동으로 도출합니다.'
            },
            en: {
                step1: 'Design modifications and analyses are repeated to meet the target performance. <br> This process generates dozens to hundreds of iterations.',
                step2: 'AslanX performs the repetitive design exploration process internally through AI.',
                step3: 'Designs generated by AI are passed to the validation stage. <br> Engineers can immediately review optimized results without repetitive iterations.',
                step4: 'AslanX generates diverse design candidates and predicts performance through AI-based design exploration. <br> AI iterates through generation, prediction, and optimization to automatically derive designs that meet target conditions.'
            },
            ja: {
                step1: '目標性能を満たすために設計修正と解析が繰り返されます。<br> このプロセスは数十回から数百回のイテレーションを発生させます。',
                step2: 'AslanXが反復的な設計探索プロセスをAIで内部的に実行します。',
                step3: 'AIが生成した設計は検証段階に渡され、<br> エンジニアは反復作業なしに最適化された結果を即座に確認できます。',
                step4: 'AslanXはAIベースの設計探索を通じて多様な設計候補を生成し、性能を予測します。<br> AIが生成・予測・最適化を繰り返し、目標条件を満たす設計を自動的に導出します。'
            }
        };
        var mslEffectTxt = mslEffectTxts[mslLang] || mslEffectTxts.en;
        var mslEffectTxtP = mslSection.querySelector('.msl_effect_txt p');
        var originalTxt = mslEffectTxtP ? mslEffectTxtP.innerHTML : '';
        var introTxt = mslEffectTxt.step1;
        ...
        if(mslEffectTxtP){
            mslTl.to(mslEffectTxtP, {
                ...
                duration: txtOutDur,
                ease: 'power2.out',
                onComplete: function(){ mslEffectTxtP.innerHTML = introTxt; },
        ...
        if(mslEffectTxtP){
            ...
            mslTl.to(mslEffectTxtP, {
                ...
                onReverseComplete: function(){ mslEffectTxtP.innerHTML = introTxt; }
            });
            mslTl.to(mslEffectTxtP, {
                opacity: 1,
                duration: txtInDur,
        ...
        if(step1Lis.length >= 3){
            ...
            if(mslEffectTxtP){
                var newTxt2 = mslEffectTxt.step3;
                mslTl.to(mslEffectTxtP, {
                    opacity: 0,
                    duration: txt2OutDur,
                    ease: 'power2.out',
        ...
        if(effectStep1 && effectStep2){
            ...
            // 텍스트 3차 변경 (공통)
            if(mslEffectTxtP){
                var newTxt3 = mslEffectTxt.step4;
                mslTl.to(mslEffectTxtP, {
                    opacity: 0,
                    duration: txt3OutDur,
                    ease: 'power2.out',
    ...
    (function(){
        ...
        var asnStep1 = asnSection.querySelector('.asn_ts_step1');
        var asnStep1Txt = asnSection.querySelector('.asn_ts_step1_txt');
        var asnStep1Start = 3;
        var asnStep1End = 5;
        var asnStep1LastP = -1;

        var asnStep2Txt = asnSection.querySelector('.asn_ts_step2_txt');
        var asnStep2InStart = 5.5;
        var asnStep2OutEnd = 9;
        var asnStep2PhaseLen = (asnStep2OutEnd - asnStep2InStart) / 3;
        var asnStep2InEnd = asnStep2InStart + asnStep2PhaseLen;
        var asnStep2OutStart = asnStep2OutEnd - asnStep2PhaseLen;
        var asnStep2LastOp = -1;
        var asnStep2LastVis = null;
        ...
        function asnSmoothUpdate(){
            ...
            if(asnStep1 || asnStep1Txt){
                var step1P = (asnCurrentTime - asnStep1Start) / (asnStep1End - asnStep1Start);
                if(step1P < 0) step1P = 0;
                else if(step1P > 1) step1P = 1;
                if(Math.abs(step1P - asnStep1LastP) > 0.0005 || (step1P >= 1) !== (asnStep1LastP >= 1)){
                    asnStep1LastP = step1P;
                    if(asnStep1Txt){
                        asnStep1Txt.style.transform = 'translateY(' + (-100 * step1P) + 'vh)';
                    }
                    if(asnStep1){
                        asnStep1.style.opacity = String(1 - step1P);
                        asnStep1.style.visibility = (step1P >= 1) ? 'hidden' : 'visible';
                    }
                }
            }

            if(asnStep2Txt){
                var t2 = asnCurrentTime;
                var step2Op, step2Vis;
                if(t2 >= asnStep2OutEnd){
                    step2Op = 0;
                    step2Vis = false;
                } else if(t2 >= asnStep2OutStart){
                    step2Op = 1 - (t2 - asnStep2OutStart) / (asnStep2OutEnd - asnStep2OutStart);
                    step2Vis = true;
                } else if(t2 >= asnStep2InEnd){
                    step2Op = 1;
                    step2Vis = true;
                } else if(t2 >= asnStep2InStart){
                    step2Op = (t2 - asnStep2InStart) / (asnStep2InEnd - asnStep2InStart);
                    step2Vis = true;
                } else {
                    step2Op = 0;
                    step2Vis = false;
                }
                if(Math.abs(step2Op - asnStep2LastOp) > 0.0005 || step2Vis !== asnStep2LastVis){
                    asnStep2LastOp = step2Op;
                    asnStep2LastVis = step2Vis;
                    asnStep2Txt.style.opacity = String(step2Op);
                    asnStep2Txt.style.visibility = step2Vis ? 'visible' : 'hidden';
                }
            }
